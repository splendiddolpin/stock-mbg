                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('beneficiaries.update', $beneficiary->id) }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="school_name" value="{{ old('school_name', $beneficiary->school_name) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4 bg-blue-50/50 p-4 rounded-lg border border-blue-100">
                            <div>
                                ...
                                <input type="number" name="porsi_besar" value="{{ old('porsi_besar', $beneficiary->porsi_besar) }}" min="0" required class="mt-1 block w-full rounded-md border-blue-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 font-bold text-blue-700">
                            ...
                            <div>
                                ...
                                <input type="number" name="porsi_kecil" value="{{ old('porsi_kecil', $beneficiary->porsi_kecil) }}" min="0" required class="mt-1 block w-full rounded-md border-blue-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 font-bold text-blue-700">
                        ...
                        <div x-data="{ 
                            search: '',
                            items: {{ $items->map(function($item) use ($beneficiary) {
                                $pivot = $beneficiary->allergens->where('id', $item->id)->first();
                                return [
                                    'id' => $item->id,
                                    'name' => $item->name,
                                    'count' => $pivot ? $pivot->pivot->anak_count : 1
                                ];
                            })->toJson() }},
                            selected: {{ $beneficiary->allergens->pluck('id')->map(fn($id) => (string)$id)->toJson() }},
                            get filteredItems() {
                                    ...
                                    <div class="flex items-center justify-between bg-white p-3 rounded-lg border transition-all shadow-sm group"
                                        ...
                                        <div x-show="selected.includes(item.id.toString())" x-collapse>
                                            <div class="flex items-center gap-1.5 border border-red-200 bg-white rounded p-1 shadow-sm">
                                                ...
                                                <input type="number" :name="`allergen_counts[${item.id}]`" min="1" :value="item.count" 
                        ...
                        <div class="flex items-center gap-4 mt-8 border-t pt-6">
                            <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 px-6 rounded-lg shadow transition-colors">Update Data Sekolah</button>
