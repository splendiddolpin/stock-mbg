            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl p-6 border border-gray-100">
                ...
                <form action="{{ route('transactions.updateIn', $transaction->id) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <p class="text-xs text-orange-500 mt-1 font-medium">Catatan: Mengubah barang akan otomatis menyesuaikan stok barang lama dan barang baru.</p>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="mb-4">
                            <label class="block text-gray-700 text-sm font-bold mb-2">Jumlah Masuk</label>
                            <input type="number" step="0.01" name="quantity" value="{{ old('quantity', $transaction->quantity) }}" class="w-full border-gray-300 rounded-xl shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                        </div>

                        <div class="mb-4">
                            <label class="block text-gray-700 text-sm font-bold mb-2">Tanggal Masuk</label>
                            <input type="date" name="date" value="{{ old('date', $transaction->date) }}" class="w-full border-gray-300 rounded-xl shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                        </div>
                    </div>

                    <div class="mb-4">
                        <label class="block text-gray-700 text-sm font-bold mb-2">Keterangan / Supplier</label>
                        <textarea name="description" rows="3" class="w-full border-gray-300 rounded-xl shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('description', $transaction->description) }}</textarea>
                    </div>
